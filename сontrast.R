...
# Получаем имя вида из полигона, если оно есть
taxon_name <- if ("SCI_NAME" %in% colnames(polygon)) {
  gsub(" ", "_", polygon$SCI_NAME[1])
} else {
  "species"
}

# Список всех ковариат (исключая 'nf')
all_covariates <- setdiff(colnames(combined_data), "nf")

# Разделяем данные по группам nf
group0 <- combined_data[combined_data$nf == 0, all_covariates, drop = FALSE]
group1 <- combined_data[combined_data$nf == 1, all_covariates, drop = FALSE]

# Фрейм для хранения статистики
gen_stats <- data.frame(
  Variable = all_covariates,
  Importance = NA,
  M_nf0 = colMeans(group0, na.rm = TRUE),
  SD_nf0 = apply(group0, 2, sd, na.rm = TRUE),
  SE_nf0 = apply(group0, 2, function(x) sd(x, na.rm = TRUE) / sqrt(length(na.omit(x)))),
  M_nf1 = colMeans(group1, na.rm = TRUE),
  SD_nf1 = apply(group1, 2, sd, na.rm = TRUE),
  SE_nf1 = apply(group1, 2, function(x) sd(x, na.rm = TRUE) / sqrt(length(na.omit(x)))),
  T_Statistic = NA,
  P_Value = NA,
  stringsAsFactors = FALSE
)

# Расчёт t-статистик и p-значений
for (i in seq_along(all_covariates)) {
  x0 <- group0[[i]]
  x1 <- group1[[i]]
  test_result <- tryCatch(t.test(x0, x1), error = function(e) NULL)
  if (!is.null(test_result)) {
    gen_stats$T_Statistic[i] <- test_result$statistic
    gen_stats$P_Value[i] <- ifelse(test_result$p.value < 0.0001, "< 1e-04", sprintf("%.4f", test_result$p.value))
  }
}

# Добавляем финальные строки с количеством наблюдений
n_nf0 <- sum(combined_data$nf == 0)
n_nf1 <- sum(combined_data$nf == 1)
n_total <- nrow(combined_data)

gen_stats_with_counts <- rbind(
  gen_stats,
  data.frame(
    Variable = c("Number of observations (nf=0)", "Number of observations (nf=1)", "Total observations"),
    Importance = NA,
    M_nf0 = c(n_nf0, NA, n_total),
    SD_nf0 = NA, SE_nf0 = NA,
    M_nf1 = c(NA, n_nf1, NA),
    SD_nf1 = NA, SE_nf1 = NA,
    T_Statistic = NA, P_Value = NA
  ),
  data.frame(
    Variable = paste("Note: Importance column is not available in this analysis."),
    Importance = NA,
    M_nf0 = NA, SD_nf0 = NA, SE_nf0 = NA,
    M_nf1 = NA, SD_nf1 = NA, SE_nf1 = NA,
    T_Statistic = NA, P_Value = NA
  )
)

# Переименуем колонки
colnames(gen_stats_with_counts) <- c(
  "Variable", "Importance", "M (nf=0)", "SD (nf=0)", "SE (nf=0)", 
  "M (nf=1)", "SD (nf=1)", "SE (nf=1)", "T-Statistic", "P-Value"
)

# Округляем
gen_stats[, 3:9] <- round(gen_stats[, 3:9], 3)

# Сохраняем в файл
output_filename <- file.path(getwd(), paste0(taxon_name, "_general_stats.xlsx"))

tryCatch({
  write_xlsx(gen_stats_with_counts, path = output_filename)
  cat("\nTable successfully exported to:", output_filename, "\n")
}, error = function(e) {
  cat("\nError exporting to Excel:", e$message, "\n")
  csv_filename <- file.path(getwd(), paste0(taxon_name, "_general_stats.csv"))
  write.csv(gen_stats_with_counts, csv_filename, row.names = FALSE)
  cat("Saved as CSV instead:", csv_filename, "\n")
})

# Очистка временных файлов
unlink(list.files(temp_dir, pattern = "^(cropped_rasters|sre_mask|temp_mask)", full.names = TRUE))